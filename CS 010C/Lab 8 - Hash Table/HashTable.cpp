...
int HashTable::computeHash(const string &s) {
	return hash<string>()(s) % size;		//uses the hash function to calculate hash of string, 
											//and take modulo with size so it fits within the array.
}
...
void HashTable::put(const string &s, int score) {
	...
	for(auto& entry : hashTable[index]) {	//iterates through list
		if(entry.getWord() == s) {			//checks if word is same as string
...
double HashTable::getAverage(const string &s) {
	...
	for(auto& entry : hashTable[index]) {
		if(entry.getWord() == s) {
			return entry.getAverage();	//returns the average sentiment of the entry
